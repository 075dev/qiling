...
子代理类型的可见性由宿主决定,**不保证每个嵌套上下文都有**。派发矩阵:

| 派发者上下文 | 可派发 | 说明 |
|--------------|--------|------|
| 主会话 | ql-builder-coordinator、ql-reviewer | 编排模式入口;独立评审直接从主会话派 |
| 协调器(子代理) | ql-builder-worker | **当且仅当宿主把该类型暴露给子代理上下文**——部分宿主只在主会话暴露子代理类型 |
| worker / reviewer | (无) | 叶子角色,不再派发 |

**通道断裂的处理契约:**

- 协调器发现 Agent 工具里没有 `ql-builder-worker` → 立即返回 `DISPATCH_CHANNEL_UNAVAILABLE` 并停止
- **绝不自行降级到外部 CLI/子进程派发**(agent CLI、`--yolo` 类无人值守通道尤其禁止)——不受控通道的配额、安全、行为均无保障,两轮死于 429 再花一小时诊断,是真实付过的学费
- 主会话收到该信号 → 改内联模式(≤ inline_threshold 时本就合理)或换宿主,决定权在拥有全局上下文的一方
...
四条规则只推导**任务间**依赖,漏掉**接口被多方消费**的横向耦合:某个任务的 Produces(如 `dispatch` 签名、公共 Envelope 类型)被 ≥2 个后续任务 Consumes 时,它就是共享接口——不定稿就放后续任务并行开工,各 worker 会撞到同一堵墙后各自绕行(`as unknown as`、类型断言、各自报告),消解只能靠合并时碰运气。

处理:**共享接口的定稿前移到 Wave 1**(显式任务 + 精确 Produces 声明),或拆出独立的"接口定稿"任务只产签名/类型与最小桩。分析结果写进阶段报告供评审核对(详见 agents/ql-builder-coordinator.md 步骤 3)。
