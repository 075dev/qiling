...
下一步:审阅 .qiling/docs/ 索引,确认覆盖;之后 /ql-design 进入 API 契约生成。
```
...
| 文件 | 由谁生成 | 何时 |
|------|----------|------|
| `.qiling/docs/chapters/chapter-01-*.md` | `/ql-scan` | 项目初始化 |
| `.qiling/docs/chapters/chapter-NN-*.md`(N≥2) | `/ql-doc` | ql-deliver 后 |
| `.qiling/docs/README.md` | **两者共同维护**(增量更新) | 任一命令触发时 |

**索引合并策略:**
- `/ql-scan` 在已有 README 末尾追加新行,不覆盖
- `/ql-doc` 在已有 README 末尾追加新行,不覆盖
...
- 若无 OpenAPI → `/ql-design` 先生成契约

## 与 GSD map-codebase 的对比

| 维度 | GSD map-codebase | ql-scan |
|------|-----------------|------------|
| 产出 | 7 份分析报告 | **1 章节文档 + 索引更新** |
| 格式 | STACK.md / ARCHITECTURE.md 等 | 与 ql-doc 完全相同的 5 节结构 |
