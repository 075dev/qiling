...
<objective>
通过阅读现有项目,**长出文档树**——与 `/ql-doc` 的章节留档**完全一致的格式与产出位置**:

- **章节文件**:`.qiling/docs/chapters/chapter-NN-*.md`(5 节结构,与 ql-doc 完全相同)
...
| 命令 | 触发 | 数据来源 | 章节 ID 来源 |
|------|------|----------|--------------|
| `/ql-doc` | ql-deliver 成功后 | OpenAPI + 构建/验证报告 | STATE.md `current_phase` |
| `/ql-scan` | 项目初始化 / 接手项目 / 文档树刷新 | **目录扫描**(agent 派发后)+ 现有 OpenAPI(若有)| 项目根目录中已有章节的最大编号 + 1 |
